...
def compute_masks(
    image_array: np.ndarray, config_params: Dict[str, Any]
    ...
    if config_params.get("use_otsu", True):
        try:
            ...
            green_binary = (
                green_normalized
                > float(config_params.get("green_channel_threshold", 0.2))
            ).astype(np.uint8)
        try:
            ...
            red_binary = (
                red_normalized > float(config_params.get("red_channel_threshold", 0.3))
            ).astype(np.uint8)
    else:
        green_binary = (
            green_normalized > float(config_params.get("green_channel_threshold", 0.2))
        ).astype(np.uint8)
        red_binary = (
            red_normalized > float(config_params.get("red_channel_threshold", 0.3))
        ).astype(np.uint8)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Visualize red/green channels and masks for one example image"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "-o",
        default=None,
        help="Output image path (PNG). Defaults to <output_directory>/example_channels.png",
    )
    ...
    red_channel, green_channel, grayscale_image, red_mask, green_mask = compute_masks(
        image_array, config_params
    )
